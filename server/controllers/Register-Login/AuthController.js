...
const RegisterController = async (req, res) => {
  ...
  if (!Email || !Password || !ConfirmPassword || !Role || !isValidEmail(Email))
    return res.status(400).json({
      success: false,
...
const GoogleSignUpIN = async (req, res) => {
  ...
  try {
    ...
    // Check if user exists in Login model to get the role
    const loginUser = await Login.findOne({ Email: Email });
    if (loginUser) {
      role = loginUser.Role;
    } else {
      // If not in Login model, create one with default role
      const newLogin = new Login({
        uuid: unique_id,
        Email: Email,
        Role: role,
