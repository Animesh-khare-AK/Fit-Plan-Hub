...
const UserDashBoardLoggedUser = async (req, res) => {
  ...
  try {
    ...
    const AlreadyBuyedPlan = await purchased
      .find({ PurchasedBy: Email }, { TrainerPlanId: 1, _id: 0 })
      .lean();
...
const UserDashBoardUnLoggedUser = async (req, res) => {
  const ContentToDisplay = await TrainerPlan.find(
    {}
    // { Description: 0 } // Commented out to include Description
  ).lean();
  try {
    res.status(200).json({
      success: true,
...
const showpurchasedplan = async (req, res) => {
  ...
  try {
    const findAllpurchasedPlan = await purchased.find(
      { PurchasedBy: Email },
      { TrainerPlanId: 1, _id: 0 }
    );
...
module.exports = {
  UserDashBoardLoggedUser,
  UserDashBoardUnLoggedUser,
  showSuscriberlist,
  showpurchasedplan,
  ShowDescription,
};
